            ...
            try:
                ...
                for s in range(len(specs)):
                    ...
                    if filepath:
                        ...
                        ##########added file write testing code#################vk
                        #with open(os.path.join(filepath, 'your_file.txt'), 'w') as f:
                            #for item in specs:
                            #    f.write(c + "%s\n" % item)
                            #f.write(c + "%s\n" % np.array(specs[s] * 255))
                            #print(f'Shape: {specs[s].shape}')
                            #print(f'Observation: \n{specs[s][0]}')
                        np.savetxt(os.path.join(filepath, filename + '.txt'), np.array(specs[s] * 255), delimiter=',')
